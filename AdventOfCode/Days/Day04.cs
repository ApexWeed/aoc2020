    ...
    public class Day04 : IDay
    {
        ...
        private static List<Passport> GetPassports()
        {
            ...
            foreach (var line in Input)
            {
                ...
                foreach (var pair in parts.Select(p => p.Split(':')))
                {
                    current.Fields.Add(pair[0], pair[1]);
                }
            }
        ...
        private class Passport
        {
            ...
            {
                return Fields.Count(f => f.Key != "cid") == 7;
            }
            ...
            public bool StrictValidate()
            {
                ...
                if (!int.TryParse(Fields["byr"], out var birthYear) || birthYear < 1920 || birthYear > 2002)
                    return false;

                if (!int.TryParse(Fields["iyr"], out var issueYear) || issueYear < 2010 || issueYear > 2020)
                    return false;

                if (!int.TryParse(Fields["eyr"], out var expireYear) || expireYear < 2020 || expireYear > 2030)
                    return false;

                if (Fields["hgt"].Contains("cm"))
                {
                    if (!int.TryParse(Fields["hgt"].Substring(0, Fields["hgt"].Length - 2), out var height) || height < 150 || height > 193)
                        return false;
                }
                else if (Fields["hgt"].Contains("in"))
                {
                    if (!int.TryParse(Fields["hgt"].Substring(0, Fields["hgt"].Length - 2), out var height) || height < 59 || height > 76)
                        return false;
                }
                else
                {
                ...
                if (new[] {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"}.All(c => c != Fields["ecl"]))
                    return false;
