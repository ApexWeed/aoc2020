...
namespace AdventOfCode.Days
{
    public class Day07 : IDay
    {
        ...
        {
            var rules = Input.Select(l => new Rule(l)).Where(l => l.Valid).ToList();

            var queue = new Queue<string>();
            queue.Enqueue("shiny gold");
            var validColours = new HashSet<string>();
            while (queue.Any())
            {
                var colour = queue.Dequeue();

                foreach (var rule in rules.Where(r => r.Limits.Any(l => l.Colour == colour)))
                {
                    if (validColours.Contains(rule.Colour))
                        continue;

                    validColours.Add(rule.Colour);
                    queue.Enqueue(rule.Colour);
                }
            }

            var validColoursCount = validColours.Count;
            return validColoursCount;
        }

        private class Rule
        {
            private static Regex Parser = new (@"(.*?) bags contain (.*).");
            ...
            public Rule(string line)
            {
                ...
                Valid = true;
                Colour = match.Groups[1].Value;

                Limits = match.Groups[2].Value.Split(',').Select(l => new Limit(l)).Where(l => l.Valid).ToList();
            }

            public override string ToString()
            {
                return $"{Colour} -> {{ {string.Join(", ", Limits)} }}";
            }
        }

        private class Limit
        {
            private Regex Parser = new(@"(\d+) (.*) bags?");

            public Limit(string line)
            {
                var match = Parser.Match(line);
                if (!match.Success)
                    return;
        ...
        {
            var rules = Input.Select(l => new Rule(l)).Where(l => l.Valid).ToList();
            var goldRule = rules.First(r => r.Colour == "shiny gold");

            var queue = new Queue<(Rule rule, int Multiplier)>();
            queue.Enqueue((goldRule, 1));

            var total = 0;
            while (queue.Any())
            {
                var (rule, multiplier) = queue.Dequeue();
                foreach (var limit in rule.Limits)
                {
                    total += limit.Count * multiplier;
                    queue.Enqueue((rules.First(r => r.Colour == limit.Colour), limit.Count * multiplier));
                }
            }
