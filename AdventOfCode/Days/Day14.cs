    ...
    public class Day14 : IDay
    {
        ...
        public void RunPart1(bool silent)
        {
            ...
            foreach (var line in Input)
            {
                ...
                var memMatch = Regex.Match(line, @"mem\[(.*)\] = (.*)");
                if (maskMatch.Success)
                {
                    mask = new Mask(maskMatch.Groups[1].Value);
                    continue;
                }

                var offset = int.Parse(memMatch.Groups[1].Value);
        ...
        private class Mask
        {
            private ulong _zero;
            private ulong _one;
            private List<int> _floating;

            public Mask(string raw)
            {
                _zero = Convert.ToUInt64(raw.Replace('X', '1'), 2);
                _one = Convert.ToUInt64(raw.Replace('X', '0'), 2);
                _floating = raw.Select((v, i) => (v, i)).Where(c => c.v == 'X').Select(p => p.i).ToList();
            }

            public ulong Apply(ulong value)
            {
                return value & _zero | _one;
            }

            public IEnumerable<ulong> MemoryApply(ulong address)
            {
                address |= _one;
                ...
                var permutations = Math.Pow(2, _floating.Count);
                for (short i = 0; i < permutations; i++)
                {
                    var mutation = Convert.ToString(i, 2).PadLeft(_floating.Count, '0');
                    var curr = 0;
                    var newAddress = Convert.ToString((long)address, 2).PadLeft(36, '0').ToArray();
                    foreach (var index in _floating)
                    {
                        newAddress[index] = mutation[curr++];
                    }

                    addresses.Add(Convert.ToUInt64(new string(newAddress), 2));
                }
        ...
        public void RunPart2(bool silent)
        {
            ...
            foreach (var line in Input)
            {
                ...
                var offset = ulong.Parse(memMatch.Groups[1].Value);
                var value = ulong.Parse(memMatch.Groups[2].Value);

                if (mask == null)
                    continue;

                foreach (var address in mask.MemoryApply(offset))
